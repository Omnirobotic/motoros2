...
typedef struct
{
    ...
    double maxSpeed[MP_GRP_AXES_NUM]; // maximum joint speed in radian/sec (rotational) or meter/sec (linear) (ROS
                                      // joint-order)
    ...
    JointMotionData trajectoryToProcess[Q_SIZE]; // joint motion command data in radian to process

    BOOL hasDataToProcess;  // indicates that there is data to process
    UINT64 timeLeftover_ms; // Time left over after reaching the end of a trajectory to complete the interpolation
                            // period
    long prevPulsePos[MAX_PULSE_AXES]; // The commanded pulse position that the trajectory starts at
                                       // (Ros_MotionServer_StartTrajMode)
    AXIS_MOTION_TYPE axisType;         // Indicates whether axis is rotary or linear
    char jointNames_userDefined[MP_GRP_AXES_NUM][MAX_JOINT_NAME_LENGTH]; // string name for each joint in 'moto'
                                                                         // (non-sequential) joint order
...
//  BOOL bIsLastGrpToInit: TRUE if this is the final group that is being initialized. FALSE if you plan to call this
//  function again. float interpolPeriod: Value of the interpolation period (ms) for the robot controller.
...
extern void
Ros_CtrlGroup_ConvertToRosPos(CtrlGroup* ctrlGroup, long const pulsePos[MAX_PULSE_AXES], double rosPos[MAX_PULSE_AXES]);
extern void Ros_CtrlGroup_ConvertToRosTorque(
        CtrlGroup* ctrlGroup,
        double const motoTorque[MAX_PULSE_AXES],
        double rosTorque[MAX_PULSE_AXES]);
extern void Ros_CtrlGroup_ConvertToMotoPos_FromSequentialOrdering(
        CtrlGroup* ctrlGroup,
        double const radPos[MAX_PULSE_AXES],
        long pulsePos[MAX_PULSE_AXES]);
extern void Ros_CtrlGroup_ConvertRosUnitsToMotoUnits(
        CtrlGroup* ctrlGroup,
