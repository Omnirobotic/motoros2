...
/// <param name="out_jointMotionData">Pointer to the head of the array containing the data which will be later broken
/// into increments</param> <returns></returns>
Init_Trajectory_Status Ros_MotionControl_ConvertTrajectoryToJointMotionData(
        JointTrajectoryPointSequence* in_jointTrajData,
        int incomingAxisIndex,
        CtrlGroup* ctrlGroup,
        int ctrlGroupAxisIndex,
        JointMotionData* out_jointMotionData);

BOOL Ros_MotionControl_AllGroupsInitComplete = FALSE;

MOTION_MODE Ros_MotionControl_ActiveMotionMode = MOTION_MODE_INACTIVE;

BOOL Ros_MotionControl_MustInitializePointQueue =
        TRUE; // first point of streaming trajectory must match current-position

Init_Trajectory_Status Ros_MotionControl_Init(
        JointsName* sequenceGoalJointNames,
        JointTrajectoryPointSequence* sequenceOfPoints)
{
    ...
    for (grpIndex = 0; grpIndex < g_Ros_Controller.numGroup; grpIndex += 1)
    {
        ...
        bzero(g_Ros_Controller.ctrlGroups[grpIndex]->trajectoryToProcess,
              sizeof(JointMotionData) * MAX_NUMBER_OF_POINTS_PER_TRAJECTORY);
    }

    //------------------------------------------------------------
    // The trajectory contains information for all groups. Determine which groups are used by looking at the 'joint
    // names'.
    ...
    // This allows it to find the correct CtrlGroup object and the joint index (in moto order) in the JointMotionData
    // array.
    //===================================
    ...
    for (jointIndexInTraj = 0; jointIndexInTraj < sequenceGoalJointNames->size; jointIndexInTraj += 1)
    {
        ...
        if (!bFound)
        {
            ...
            {
                int jointIndex;
                for (jointIndex = 0; jointIndex < MP_GRP_AXES_NUM; jointIndex += 1)
                {
        ...
        Init_Trajectory_Status convertStatus = Ros_MotionControl_ConvertTrajectoryToJointMotionData(
                sequenceOfPoints,
                jointIndexInTraj,
                ctrlGroup,
                jointIndexInCtrlGroup,
                ctrlGroup->trajectoryToProcess);
    ...
    for (grpIndex = 0; grpIndex < g_Ros_Controller.numGroup; grpIndex += 1)
    {
        ...
        {
            int i;
            for (i = 0; i < sequenceOfPoints->size; i += 1)
            {
                // This is radians in MOTO joint order
                ctrlGroup->trajectoryToProcess[i].pos[4] +=
                        -ctrlGroup->trajectoryToProcess[i].pos[1] + ctrlGroup->trajectoryToProcess[i].pos[2];
                ctrlGroup->trajectoryToProcess[i].vel[4] +=
                        -ctrlGroup->trajectoryToProcess[i].vel[1] + ctrlGroup->trajectoryToProcess[i].vel[2];
            }
        }
        ...
        // Although there are additional groups to process, we'll set this flag to indicate that the point is ready for
        // processing. The Ros_MotionControl_AllGroupsInitComplete flag has not been set yet. That will prevent the
        // AddToIncQueueProcess loop from processing increments before all groups are synchronized.
        for (i = 0; i < sequenceOfPoints->size; i += 1)
        {
...
void Ros_MotionControl_AddToIncQueueProcess(CtrlGroup* ctrlGroup)
{
    ...
    while (TRUE)
    {
        if (Ros_MotionControl_AllGroupsInitComplete)
        {
            ...
            if (ctrlGroup->hasDataToProcess && ctrlGroup->trajectoryIterator != NULL &&
                ctrlGroup->trajectoryIterator->valid)
            {
                ...
                // Set the start of the trajectory interpolation as the current position (which should be the end of
                // last interpolation)
                ...
                if (interval > 0.0)
                {
                    for (i = 0; i < ctrlGroup->numAxes; i++)
                    {
                        ...
                        accCoef2[i] = (-12 * (endTrajData->pos[i] - startTrajData->pos[i]) /
                ...
                while ((curTrajData->time < endTrajData->time) && Ros_Controller_IsMotionReady())
                {
                    ...
                    if (calculationTime_ms < endTrajData->time) // Make calculation for full interpolation clock
                    {
                        ...
                        for (i = 0; i < ctrlGroup->numAxes; i++)
                        {
                            ...
                            curTrajData->pos[i] = startTrajData->pos[i]                  // initial position component
                                                  ...
                                                  + accCoef2[i] * interpolTime * interpolTime * interpolTime /
                                                            6; // accCoef2 component
                    ...
                    else // Make calculation for partial interpolation cycle
                    {
                        ...
                        // Set the next interpolation increment to the the remainder to reach the next interpolation
                        // cycle
                ...
                {
                    if (ctrlGroup->trajectoryIterator ==
                        &ctrlGroup->trajectoryToProcess
                                 [MAX_NUMBER_OF_POINTS_PER_TRAJECTORY]) // pointing to last possible entry in the array;
...
UINT8 Ros_MotionControl_ProcessQueuedTrajectoryPoint(QueueTrajPoint* request)
{
    ...
    for (jointIndexInTraj = 0; jointIndexInTraj < request->joint_names.size; jointIndexInTraj += 1)
    {
        ...
        // `Ros_MotionControl_Init` function
        //      populated the first buffer position with the initial point in the queue. Followup points are placed in
        //      the second buffer position. As the destination in position 2 is processed, it is moved into position 1
        //      to become the starting point for the next destination.
...
void Ros_MotionControl_IncMoveLoopStart() //<-- IP_CLK priority task
{
    ...
    // When FSU speed limitation is active, some pulses for an interpolation cycle may not be processed by the
    // controller. To track the true amount of pulses processed, we keep track of the command position and by
    // substracting the the previous position from the current one, we can confirm the amount if pulses precessed. If
    // the amount processed doesn't match the amount sent at the previous cycle, we resend the unprocessed pulses
    // amount. To keep the motion smooth, the 'maximum speed' (max pulses per cycle) is tracked and used to skip reading
    ...
    LONG newPulseInc[MAX_CONTROLLABLE_GROUPS]
                    [MP_GRP_AXES_NUM]; // Pulse increments that we just retrieved from the incQueue
    LONG toProcessPulses[MAX_CONTROLLABLE_GROUPS]
                        [MP_GRP_AXES_NUM]; // Total pulses that still need to be sent to the command
    LONG processedPulses[MP_GRP_AXES_NUM]; // Amount of pulses from the last command that were actually processed
                                           // (accepted)
    LONG maxSpeed[MAX_CONTROLLABLE_GROUPS][MP_GRP_AXES_NUM]; // ROS speed (amount of pulses for one cycle from the data
                                                             // queue) that should not be exceeded
    LONG maxSpeedRemain[MAX_CONTROLLABLE_GROUPS]
                       [MP_GRP_AXES_NUM]; // Number of pulses (absolute) that remains to be processed at the 'maxSpeed'
    ...
    BOOL skipReadingQ[MAX_CONTROLLABLE_GROUPS]; // Flag indicating to skip reading more data from the increment queue
                                                // (there is enough unprocessed from previous cycles remaining)
    BOOL queueRead[MAX_CONTROLLABLE_GROUPS]; // Flag indicating that new increment data was retrieve from the queue on
                                             // this cycle.
    BOOL isMissingPulse; // Flag that there are pulses send in last cycle that are missing from the command (pulses were
                         // not processed)
    BOOL hasUnprocessedData; // Flag that at least one axis (any group) still has unprecessed data. (Used to continue
                             // sending data after the queue is empty.)
    ...
    FOREVER
    {
        ...
        if (Ros_Controller_IsMotionReady() && (Ros_MotionControl_HasDataInQueue() || hasUnprocessedData) &&
            !g_Ros_Controller.bStopMotion)
        {
            ...
            for (i = 0; i < g_Ros_Controller.numGroup; i++)
            {
                ...
                for (axis = 0; axis < MP_GRP_AXES_NUM; axis++)
                {
                    ...
                    // If everything was processed, then there should by 0 pulses left. Otherwise FSU Speed limit
...
{
    int groupIndex;
    for (groupIndex = 0; groupIndex < g_Ros_Controller.numGroup; groupIndex++)
    {
...
{
    return (Ros_Controller_IsInMotion() && (Ros_MotionControl_ActiveMotionMode != MOTION_MODE_INACTIVE));
}
...
BOOL Ros_MotionControl_StopMotion(BOOL bKeepJobRunning)
{
    ...
    for (checkCnt = 0; checkCnt < MOTION_STOP_TIMEOUT; checkCnt++)
    {
        ...
        int i;
        for (i = 0; i < g_Ros_Controller.numGroup; i += 1)
        {
    ...
    for (grpNo = 0; grpNo < g_Ros_Controller.numGroup; ++grpNo)
    {
        if (g_Ros_Controller.ctrlGroups[grpNo] != NULL)
        {
            Ros_CtrlGroup_GetPulsePosCmd(
                    g_Ros_Controller.ctrlGroups[grpNo],
                    g_Ros_Controller.ctrlGroups[grpNo]->prevPulsePos);
        }
    }
...
{
    return (Ros_MotionControl_ActiveMotionMode == MOTION_MODE_TRAJECTORY);
}

BOOL Ros_MotionControl_IsMotionMode_PointQueue()
{
    return (Ros_MotionControl_ActiveMotionMode == MOTION_MODE_POINTQUEUE);
}
...
{
    if (!g_messages_RobotStatus.msgRobotStatus->motion_possible)
    {
        if (Ros_MotionControl_IsMotionMode_PointQueue())
        {
