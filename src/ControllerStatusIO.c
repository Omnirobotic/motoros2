...
BOOL Ros_Controller_Initialize()
{
    ...
    for (groupIndex = 0; groupIndex < MAX_CONTROLLABLE_GROUPS; groupIndex++)
    {
        if (groupIndex < g_Ros_Controller.numGroup)
        {
            ...
            g_Ros_Controller.ctrlGroups[groupIndex] = Ros_CtrlGroup_Create(
                    groupIndex, // Zero based index of the group number(0 - 3)
                    (groupIndex ==
                     (g_Ros_Controller.numGroup - 1)), // TRUE if this is the final group that is being initialized.
                                                       // FALSE if you plan to call this function again.
                    g_Ros_Controller
                            .interpolPeriod); // Value of the interpolation period (ms) for the robot controller.
            if (g_Ros_Controller.ctrlGroups[groupIndex] != NULL)
            {
                Ros_CtrlGroup_GetPulsePosCmd(
                        g_Ros_Controller.ctrlGroups[groupIndex],
                        g_Ros_Controller.ctrlGroups[groupIndex]->prevPulsePos); // set the current commanded pulse
    ...
    {
        printf("Eco-mode: %sabled", eco_mode_info.bEnabled ? "en" : "dis");
        if (eco_mode_info.bEnabled)
        {
            printf("Eco-mode: timeout: %u %s",
...
void Ros_Controller_Cleanup()
{
    ...
    for (groupNum = 0; groupNum < MAX_CONTROLLABLE_GROUPS; groupNum += 1)
    {
        ...
        {
            printf("Cleanup control group %d", groupNum + 1);
...
void Ros_Controller_StatusInit()
{
    ...
    g_Ros_Controller.ioStatusAddr[IO_ROBOTSTATUS_PFL_AVOID_JOINT].ulAddr =
            15124; // PFL function avoidance joint enabled
    g_Ros_Controller.ioStatusAddr[IO_ROBOTSTATUS_PFL_AVOID_TRANS].ulAddr =
            15125; // PFL function avoidance translation enabled
#endif
...
{
    return ((g_Ros_Controller.ioStatus[IO_ROBOTSTATUS_SERVO] != 0) && !Ros_Controller_IsEcoMode());
}
...
BOOL Ros_Controller_IsPflActive()
{
    ...
    if (g_Ros_Controller.bPFLEnabled)
    {
        if (g_Ros_Controller.bPFLduringRosMove || g_Ros_Controller.ioStatus[IO_ROBOTSTATUS_PFL_STOP] ||
            g_Ros_Controller.ioStatus[IO_ROBOTSTATUS_PFL_ESCAPE] ||
            (g_Ros_Controller.ioStatus[IO_ROBOTSTATUS_PFL_AVOIDING] &&
             (g_Ros_Controller.ioStatus[IO_ROBOTSTATUS_PFL_AVOID_JOINT] ||
              g_Ros_Controller.ioStatus[IO_ROBOTSTATUS_PFL_AVOID_TRANS])) != 0)
        {
            return TRUE;
        }
    }
...
BOOL Ros_Controller_IsAnyFaultActive()
{
    ...
    return (Ros_Controller_IsAlarm() || Ros_Controller_IsError() || Ros_Controller_IsMpIncMoveErrorActive() ||
    ...
    if (Ros_Controller_StatusRead(ioStatus))
    {
        ...
        for (i = 0; i < IO_ROBOTSTATUS_MAX; i++)
        {
            if (g_Ros_Controller.ioStatus[i] != ioStatus[i])
            {
                ...
                {
                    if ((ioStatus[IO_ROBOTSTATUS_ALARM_MAJOR] == 0) && (ioStatus[IO_ROBOTSTATUS_ALARM_MINOR] == 0) &&
                        (ioStatus[IO_ROBOTSTATUS_ALARM_SYSTEM] == 0) && (ioStatus[IO_ROBOTSTATUS_ALARM_USER] == 0))
                    {
                        g_Ros_Controller.alarmCode = 0;
                ...
                case IO_ROBOTSTATUS_PFL_AVOIDING: // PFL Avoidance
                {
                    if (g_Ros_Controller.bPFLEnabled && Ros_Controller_IsWaitingRos() && Ros_Controller_IsPflActive())
                    {
                        ...
                        g_Ros_Controller.bPFLduringRosMove =
                                TRUE; // force job to be restarted with new ROS_CMD_START_TRAJ_MODE command
                        Ros_MotionControl_ClearQ_All();
                    }
                    break;
                }
        ...
        if (!Ros_Controller_IsMotionReady())
        {
            ...
            else
            {
                ...
                printf("Alarms:\n");
                int alm;
                for (alm = 0; alm < num_alarms; ++alm)
                {
                    g_messages_RobotStatus.msgRobotStatus->error_codes[alm] = active_alarms[alm];
                    printf("%hu\n", active_alarms[alm]);
                }
            }
        }
...
void Ros_Controller_SetIOState(ULONG signal, BOOL status)
{
    ...
    if (ret != 0)
        printf("mpWriteIO failure (%d)", ret);
}
...
BOOL Ros_Controller_ShouldWarnNoCalibDataLoaded(
        Controller const* controller,
        BOOL bCalibLoadedOk,
        BOOL bPublishTfEnabled)
{
    ...
    return (
            ...
            TRUE == bPublishTfEnabled && FALSE == bCalibLoadedOk);
}
